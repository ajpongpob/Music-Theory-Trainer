...
ctx.window.majorScaleTrainerStartForAuthenticatedUser=async level=>{
  launches.push(level);
...
(async()=>{
  for(const code of ['MAJOR_SCALE_NOTATION','major_scale_notation',' MAJOR_SCALE_NOTATION ']){
    ...
    assert.equal(launches.at(-1),2);assert.equal(get('trainerApp').hidden,false);
  ...
  release=true;
  const first=host.launch({exerciseCode:'major_scale_notation',stageCode:'STAGE_3'});
  ...
  assert.equal(launches.length,before+1);assert.equal(host.getCurrentContext(),null);
  ...
  assert(closes>=6);
  console.log('PASS Exercise Host routing, context, single/double launch, unavailable, close/reopen, race and rollback');
