...
begin
  ...
  where ps.user_id=v_user_id
    and ps.exercise_id=v_exercise_id
    and ps.stage_id=v_stage_id
  ...
  if v_ev.mastery_passed=true then
    return query select v_path_code,v_exercise_code,v_stage_code,'advance'::text,null::text,null::text,
      'STAGE_MASTERY_READY'::text,'คุณผ่านเกณฑ์ของระดับนี้แล้ว ไประดับถัดไปได้'::text,
  ...
  if v_practice_count=0 then
    return query select v_path_code,v_exercise_code,v_stage_code,'continue'::text,null::text,null::text,
      'START_PRACTICE'::text,'เริ่มทำแบบฝึกของระดับนี้ตามเงื่อนไขหลักของระบบ'::text,
      v_ev.overall_score,v_ev.overall_threshold,coalesce(v_ev.attempts_found,0),coalesce(v_ev.rolling_window,0);
    return;
  end if;
  ...
  if v_ev.enough_attempts is not true then
    return query select v_path_code,v_exercise_code,v_stage_code,'continue'::text,null::text,null::text,
      ...
      format('ทำแบบฝึกเพิ่มอีก %s ข้อ เพื่อให้ระบบมีผลการฝึกเพียงพอสำหรับประเมินระดับนี้',greatest(v_ev.rolling_window-v_ev.attempts_found,0)),
      v_ev.overall_score,v_ev.overall_threshold,v_ev.attempts_found,v_ev.rolling_window;
    return;
  end if;
