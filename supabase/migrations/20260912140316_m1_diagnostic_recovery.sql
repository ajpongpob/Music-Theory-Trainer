-- M1.3 Diagnostic recovery
-- Keep incomplete pre-tests in the diagnostic journey rather than silently
-- converting them into practice recommendations. Also surface the completed
-- diagnostic score/threshold when practice is recommended.
-- No diagnostic/mastery threshold or placement rule changes.
...
begin
  ...
  select ps.planned_questions,ps.completed_questions
  into v_latest_pretest_planned,v_latest_pretest_completed
  ...
  if v_practice_count=0 and v_pretest_count>0 and (
    coalesce(v_latest_pretest_planned,0)<=0
    or coalesce(v_latest_pretest_completed,0)<v_latest_pretest_planned
  ) then
    return query select v_path_code,v_exercise_code,v_stage_code,'diagnostic'::text,null::text,null::text,
      'INCOMPLETE_DIAGNOSTIC'::text,
      case
        when coalesce(v_latest_pretest_planned,0)>0 then
