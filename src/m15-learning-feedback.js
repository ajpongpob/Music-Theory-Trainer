...
let names=new Map(fallback),sessionId=null,sessionLookup=null,finalizing=false,studentToken=0,teacherToken=0,studentTimer=null,teacherTimer=null;
...
async function student(){const dash=document.getElementById('studentDashboard'),content=document.getElementById('dashboardContent');if(!dash||dash.hidden||!content||content.hidden||!repo||dash.classList.contains('dashboard-v2-enabled'))return;const t=++studentToken;try{const [dr,dg]=await Promise.all([repo.getStudentDashboard(),repo.getLatestDiagnosticFeedback(EXERCISE_CODE)]);if(t!==studentToken||dr.error)return;const cur=(Array.isArray(dr.data)?dr.data:[]).find(x=>x.stage_status==='in_progress');const body=document.getElementById('dashboardMasteryBody');if(cur&&body){const er=await repo.getStageEvidence({exerciseCode:cur.exercise_code,stageCode:cur.stage_code});if(t!==studentToken||er.error)return;const ev=first(er.data);if(ev){const m=Array.isArray(ev.missing_item_codes)?ev.missing_item_codes:[];body.className='';body.innerHTML=`<div class="m15-grid"><div class="m15-stat"><strong>${pct(ev.overall_score)}</strong><span>Mastery สะสม · เกณฑ์ ${pct(ev.overall_threshold)}</span></div><div class="m15-stat"><strong>${ev.attempts_found||0} / ${ev.rolling_window||0}</strong><span>ผลการฝึกที่ใช้ประเมิน</span></div><div class="m15-stat"><strong>${ev.covered_items||0} / ${ev.required_items||0}</strong><span>บันไดเสียงที่ต้องครอบคลุม</span></div></div><div class="m15-reason"><strong>ทำไมจึง${ev.mastery_passed===true?'ผ่าน':'ยังไม่ผ่าน'}ขั้นนี้</strong><div>${esc(reason(ev))}</div>${m.length?`<div class="m15-missing">ยังขาด: ${m.map(keyName).map(esc).join(' · ')}</div>`:''}</div><div class="m15-skills">${skillRows(ev.skill_results)}</div>`}}
...
function schedule(){normalize();review();clearTimeout(studentTimer);clearTimeout(teacherTimer);studentTimer=setTimeout(student,300);teacherTimer=setTimeout(teacher,300)}
